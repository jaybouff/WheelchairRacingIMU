...
Direction=menu('Quelle est la direction du champ de force?','Flexion plantaire (résistif)','Flexion dorsale (assistif)');
...
if Cycle_Table(STIM(i),3)==1
    ...
    if Direction==1
    themax=find(delta(themaxCF+1:x(end))==min(delta(themaxCF+1:x(end)))); %themax=find((delta(themaxCF+1:end))==max(abs(delta(themaxCF+1:end)))); changé par JB remove abs
    elseif Direction==2
