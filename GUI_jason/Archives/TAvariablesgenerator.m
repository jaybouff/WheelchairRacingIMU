...
function [TA, baseline2, cycleID, CHAMPend, BASELINE2end, POSTend, RMSburstTA, normRMSburstTA, MEANburstTA, normMEANburstTA, dureeswing]= TAvariablesgenerator(Cycle_Table,data)
...
for i=1:n
    ...
    baseline2(:,i)=mean(TA.baseline2(1:1000,temp(end-99:end),i),2);
    clear temp
...
for i=1:n
    ...
    plot(mean(TA.CHAMP(:,temp(end-9:end),i),2),'m')
    plot(mean(TA.CHAMP(:,temp(2:11),i),2),'r')
    
    temp=find(isnan(TA.POST(1,:,i)')==0);
    plot(mean(TA.POST(:,temp(2:11),i),2),'g')
    ...
    MEANburstTA.baseline2(j,i)=mean(TA.baseline2(debutbouffee(i):finbouffee(i),j,i),1);
     end
    ...
    MEANburstTA.CHAMP(j,i)=mean(TA.CHAMP(debutbouffee(i):finbouffee(i),j,i),1);
    ...
    MEANburstTA.POST(j,i)=mean(TA.POST(debutbouffee(i):finbouffee(i),j,i),1);
